...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if torch.cuda.is_available() and x.device.type is not 'cpu':
            if self.use_rel_pos:
            ...
            attn = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_bias, scale=self.scale)
            x = attn.view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
...
def sam_huge(img_size=1024, pretrained=False):

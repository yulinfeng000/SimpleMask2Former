...
class ConMIMVisionTransformer(nn.Module):
    ...
    # previous method for classifier, replace it to forward feature map
    # def forward(self, x):
    ...
    def forward(self, x):
        ...
        x = nlc_to_nchw(x, self.patch_embed.patch_shape)
        return x
...
def conmim_vit_base(pretrained=False, **kwargs):
    model = ConMIMVisionTransformer(
        **arch_cfg["base"], **kwargs
    )
    if pretrained:
